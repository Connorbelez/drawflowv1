...
- Completed the final review hardening: full archives are generated asynchronously as ordered 256 KiB NDJSON storage chunks behind the same expiring, current-authorization boundary instead of a single Convex document; private follows and pins remain requester-local, seen receipts retain hierarchy filtering (including the Admin invisibility rule), moderation evidence snapshots are scrubbed through the same receipt ACL, closure now requires an active retention policy, and export evidence records the exact draw-entity and attachment/staging authorization path used.
