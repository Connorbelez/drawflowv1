...
|---|---|---|
| Active organization and route authorization | `convex/authz.ts`, `convex/activeBuildAccess.ts`, route `beforeLoad` guards | Lender management requests require one canonical active membership and supported lender role. Active-Build access preserves its existing cross-organization token, participant-grant, and canonical Build-local revocation semantics; Phase 1 does not change that owner. Deactivation removes lender-organization membership-derived access on the next request; reactivation or role change restores only currently authorized lender-management actions. |
