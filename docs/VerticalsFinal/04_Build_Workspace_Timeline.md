...
| --- | --- | --- | --- |
| **`BuildWorkspaceScreen`** (`/app/:orgSlug/builds/:buildId/workspace`) | `BuildHeader`, `MilestoneRail`, `GanttTimeline`, `DrawGroupOverlay`, `WorkspaceStatusBar`, `RoleAwareActionPanel`, `WarningSummary`, `MilestoneDrawer`, `ActionDrawer`, `PlanSummaryPanel`, `RunningCostSummaryBar` | Central control plane for an active build.  Displays approved baseline vs forecast/actual schedule, milestone cards, draw group bounding boxes, schedule health indicators, evidence/verification status, role‑aware actions, and a running cost summary that shows the total principal owed plus draw fees and interest estimates at each milestone completion or projected date.  Users can update progress, claim completion, view or upload evidence, open milestone and action drawers, monitor draw eligibility and warnings, and track how the total cost evolves over time. | PRD: Build Workspace context (Section 10); requirements for showing baseline vs forecast/actual, behind‑schedule visualization, draw group visualization, role‑aware actions, running cost summary (10.4–10.7 and Running cost summary section); user flows: Active Build Update Flow (11.3), Evidence Review Flow (11.4), Site Visit Flow (11.5), Milestone Approval and Draw Release Flow (11.6).
...
| **`RunningCostSummaryBar`** (bottom bar within `BuildWorkspaceScreen`) | `TotalCostBreakdown` | Displays a running total cost at each milestone completion or projected date, broken down into principal owed, draw fees, and interest estimates.  The interest component respects the active **InterestEstimatePolicy** (e.g., Actual/365 day‑count, daily or monthly compounding, accrual beginning the day after release, configured payoff date), and the fee component uses the **DrawFeePolicy**, reflecting whether fees are withheld from the draw or payable externally.  The bar updates automatically as forecast dates, working‑capital assumptions, or policy snapshots change.  Positioned at the bottom of the Gantt timeline to give builders and lenders real‑time visibility into the cost trajectory of the project and how fees and interest accumulate over time. | PRD: Running cost summary requirement; builder visibility into cost breakdown; ensures that the Gantt board shows cost breakdown at each milestone date.
...
The workspace must display the cumulative total cost of the project (principal owed, draw fees, and interest estimates) at each milestone completion date or projected completion date.  This running total should appear at the bottom of the Gantt timeline, giving builders and lenders real‑time visibility into the total cost and its breakdown as the schedule evolves.  The summary must update when forecast dates or working‑capital assumptions change and draw upon policy snapshots to compute fees and interest estimates.
...
export interface BuildWorkspaceProjection {
  ...
  /**
   * Running total of under‑budget amounts that can be applied to later over‑budget milestones.  This buffer pool is
   * increased when builders complete milestones under the approved budget (incurred cost < approved value) and request
   * a lower draw amount.  It is decreased when later milestones exceed their approved value and can draw from the pool.
   * Displayed in the workspace alongside the running cost summary.
   */
  bufferPoolCents?: MoneyCents;
}
```
