...
| Set borrower starting cash | `timeline-setup-cash-input`; persisted in draft package as `borrowerStartingCashCents` | No assistant action | `set_proposal_setup_field` with cents validation |
| Set Loan Percentage | `timeline-setup-loan-percentage-input`; determines reimbursable/draw availability | No assistant action | `set_proposal_setup_field` with bps validation |
