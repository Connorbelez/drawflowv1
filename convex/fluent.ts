...
export function withQueryTiming(operation: string) {
  return fluent.query().createMiddleware(async (ctx, next) => {
    ...
    try {
      ...
      console.info(`[convex] query ${operation} completed in ${Date.now() - startedAt}ms`)
    }
  })
}
...
export function withMutationTiming(operation: string) {
  return fluent.mutation().createMiddleware(async (ctx, next) => {
    ...
    try {
      ...
      console.info(`[convex] mutation ${operation} completed in ${Date.now() - startedAt}ms`)
    }
  })
}
...
export function withActionTiming(operation: string) {
  return fluent.action().createMiddleware(async (ctx, next) => {
    ...
    try {
      ...
      console.info(`[convex] action ${operation} completed in ${Date.now() - startedAt}ms`)
    }
  })
}
...
export const listTodosPage = listRecentTodos.use(withQueryTiming('fluent.listTodosPage')).public()
...
export const addTodoForViewer = authenticatedZodMutation
  ...
  .returns(v.id('todos'))
  .handler(async (ctx, args) => {
    console.info(`[convex] viewer ${ctx.viewer.tokenIdentifier} created a todo`)
