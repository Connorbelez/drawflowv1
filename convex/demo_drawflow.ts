...
const BACKOFFICE_MILESTONE_COLUMNS = [
  {
...
const BACKOFFICE_PROPOSAL_COLUMNS = [
  { id: "draft", name: "Draft" },
...
function buildDisplayId(build: Doc<"demo_builds">, prefix: string) {
...
function initials(persona: string | undefined) {
  if (!persona) {
    return undefined;
  }
...
function milestonePriority(milestone: DecoratedMilestone) {
...
function proposalColumn(status: string) {
  if (status === "approved") {
    return "approved";
  }
...
async function buildBackofficeDashboardProjection(ctx: DemoReadCtx) {
