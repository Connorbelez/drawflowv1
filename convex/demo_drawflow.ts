...
async function buildProjection(ctx: DemoMutationCtx, scenario: Scenario) {
  ...
  for (const milestone of milestones) {
    ...
    const evidencePackage = await latestEvidencePackage(
      ctx,
      scenario,
      milestone.key
    );
    const siteVisit = siteVisits
...
export const demo_submitCompletionClaim = publicMutation
  ...
  .handler(async (ctx, args) => {
    ...
    if (files.length === 0) {
      ...
      throw new Error("Completion proof is required before submission.");
    }
    if (
