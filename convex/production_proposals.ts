...
const proposalDirectoryCardValidator = v.object({
...
const productionSelectedPlanKeyInput = v.union(
  v.literal("cheapestFeasible"),
  v.literal("fastest"),
  v.literal("capitalConstrained")
);

const productionSelectedPlanMetricsInput = v.object({
  drawCount: v.number(),
...
const operationsHandoffProjectionValidator = v.object({
...
const integrationEndpointProjectionValidator = v.object({
...
const integrationDeliveryProjectionValidator = v.object({
...
const contractorProfileCreateInput = {
...
export const saveDraftProposalPackage = authenticatedMutation
  ...
  .handler(async (ctx, args) => {
    ...
    for (const costItem of args.costItems ?? []) {
      if (normalizeCostItemBudgetTreatment(costItem.budgetTreatment) !== "add") {
        continue;
      }
...
const productionSelectedPlanNames = {
  capitalConstrained: "Capital-Constrained",
  cheapestFeasible: "Cheapest Feasible",
  fastest: "Fastest",
} as const;

function normalizeSelectedPlanMetric(value: number, label: string) {
...
export const createActiveBuildCostItem = authenticatedMutation
  ...
  .handler(async (ctx, args) => {
    ...
    await applyActiveBuildCostItemBudgetDelta(ctx, auth, {
      buildId: args.buildId,
      deltaCents:
        budgetTreatment === "add" ? costItemTotalCents(item) : 0,
      milestone,
...
async function createOrReuseContractorProfile(
  ctx: MutationCtx,
  input: {
...
function activeBuildCalendarDrawRequestEvent(input: {
  build: Doc<"activeBuilds">;
...
/**
 * Assign the accountable broker for a pre-closing Build Proposal. When the
 * proposal already has a Builder, the canonical Builder relationship is
 * transferred in the same transaction so proposal access and portfolio
  ...
  .handler(async (ctx, args) => {
    ...
    const existing = await ctx.db
      ...
      .withIndex("by_build_operation", (q) =>
        q.eq("buildId", args.buildId).eq("clientOperationId", clientOperationId)
      )
      .unique();
    if (existing) {
    ...
    if (amountCents > availableCents) {
      throw invalidActiveBuildDrawRequestError({
        ...
        requestedCents: amountCents,
        safeMessage: `Requested draw exceeds the available draw limit of ${availableCents} cents.`,
      });
    }
...
async function upsertBuilderMilestoneDecisionDeliveries(
  ctx: MutationCtx,
  input: {
    auth: ActiveBuildDeliveryAuth;
    milestone: Doc<"buildMilestones">;
...
async function upsertBuilderDrawDecisionDeliveries(
  ctx: MutationCtx,
  input: {
    auth: ActiveBuildDeliveryAuth;
...
async function upsertBackofficeSiteVisitReportDeliveries(
  ctx: MutationCtx,
  input: {
    build: Doc<"activeBuilds">;
    milestone: Doc<"buildMilestones">;
...
async function resolveBuilderMilestoneDecisionDeliveries(
  ctx: MutationCtx,
  input: {
    auth: ActiveBuildDeliveryAuth;
    milestone: Doc<"buildMilestones">;
...
function operationsHandoffProjection(handoff: Doc<"operationsQueueHandoffs">) {
  return {
    _id: handoff._id,
...
async function authorizeOperationsQueueMutation(
...
async function getOperationsHandoffOrThrow(
  ctx: (QueryCtx | MutationCtx) & { viewer: AuthorizedViewer },
...
async function resolveOperationsQueueTarget(
  ctx: QueryCtx | MutationCtx,
  input: {
    auth: { brokerage: Doc<"brokerages"> };
    queueItemId: string;
...
function requireOperationsHandoffText(value: string, label: string) {
...
function normalizeOperationsHandoffWarnings(warnings: string[]) {
  return [
    ...new Set(warnings.map((warning) => warning.trim()).filter(Boolean)),
  ].slice(0, 10);
}

function operationsHandoffAuditState(handoff: Doc<"operationsQueueHandoffs">) {
...
async function writeOperationsHandoffEvent(
  ctx: MutationCtx,
  input: {
    auth: { brokerage: Doc<"brokerages">; roles: RoleSlug[]; subject: string };
    command: string;
...
function integrationEndpointProjection(endpoint: Doc<"integrationEndpoints">) {
  return {
    _id: endpoint._id,
...
function integrationDeliveryProjection(
  attempt: Doc<"integrationDeliveryAttempts">,
...
async function authorizeIntegrationAdmin(
  ctx: (QueryCtx | MutationCtx) & { viewer: AuthorizedViewer },
  workosOrganizationId: string
) {
  const auth = await authorizeBrokerage(ctx, workosOrganizationId);
...
async function getIntegrationEndpointOrThrow(
  ctx: (QueryCtx | MutationCtx) & { viewer: AuthorizedViewer },
  endpointId: Id<"integrationEndpoints">,
  workosOrganizationId: string
) {
...
async function getIntegrationDeliveryAttemptOrThrow(
  ctx: (QueryCtx | MutationCtx) & { viewer: AuthorizedViewer },
...
function requireIntegrationText(value: string, label: string) {
  const normalized = value.trim();
  if (!normalized) {
...
function requireIntegrationReason(reason: string) {
...
function normalizeIntegrationEndpointUrl(value: string) {
  const normalized = value.trim();
  let endpointUrl: URL;
  try {
...
function normalizeIntegrationEventTypes(eventTypes: string[]) {
  const normalized = [
...
function integrationEndpointAuditState(endpoint: Doc<"integrationEndpoints">) {
  return JSON.stringify({
    endpointUrl: endpoint.endpointUrl,
    eventTypes: endpoint.eventTypes,
    name: endpoint.name,
...
async function getRecipientDeliveryOrThrow(
  ctx: (QueryCtx | MutationCtx) & { viewer: AuthorizedViewer },
...
function requireApproverActiveBuildWrite(auth: {
  proposal: Doc<"buildProposals">;
  roles: RoleSlug[];
  subject: string;
}) {
...
function viewerFromBuilderStaffProvisionActor(actor: {
  organizationId?: string;
  roles: string[];
  subject: string;
}): AuthorizedViewer {
...
function withBorrowerStartingCash<
  T extends {
    borrowerStartingCashCents?: number;
    borrowerWorkingCapitalLimitCents?: number;
    timelineStartingCashCents?: number;
...
async function productionProposalDirectoryCard(
  ctx: QueryCtx | MutationCtx,
  proposal: Doc<"buildProposals">
...
async function productionProposalDirectoryMatchContext(
  ctx: QueryCtx | MutationCtx,
  proposal: Doc<"buildProposals">
) {
...
function proposalDirectoryCardMatches(input: {
  attachedBuilder?: {
  ...
  if (
    filters?.closingState === "active_build" &&
    !card.activeBuildId
  ) {
    return false;
  }
  ...
  if (!numberInRange(card.totalBudgetCents, filters?.minBudgetCents, filters?.maxBudgetCents)) {
    return false;
  }
  if (
    !numberInRange(
  ...
  if (!numberInRange(card.createdAt, filters?.createdFrom, filters?.createdTo)) {
    return false;
  }
  if (!numberInRange(card.updatedAt, filters?.updatedFrom, filters?.updatedTo)) {
    return false;
  }
  if (
    !numberInRange(
  ...
  if (
    ...
    (!card.proposedStartDate || card.proposedStartDate < filters.proposedStartFrom)
  ) {
    return false;
  }
  if (
    filters?.proposedStartTo &&
    (!card.proposedStartDate || card.proposedStartDate > filters.proposedStartTo)
  ) {
    return false;
  }
  ...
  const searchText = JSON.stringify({
    assignment: {
      ...
      builderEmail:
        attachedBuilder?.preferredEmail ?? card.builderEmail,
    ...
    formatted: {
      ...
      lenderDrawPolicyLimit: centsToCurrency(
        card.lenderDrawPolicyLimitCents
      ),
    },
    proposal: withBorrowerStartingCash(proposal),
...
function productionQueueAgeLabel(timestamp: number, asOfDate: string) {
...
function productionDaysActive(startDate: string, asOfDate?: string) {
...
function activeBuildMilestoneSummary(input: {
...
function productionMilestoneColumn(
  milestone: Doc<"buildMilestones">,
  currentDay: number
) {
...
function productionMilestoneIsBehindSchedule(
  milestone: Doc<"buildMilestones">,
  currentDay: number
) {
  if (
...
function productionMilestoneDueLabel(
  milestone: Doc<"buildMilestones">,
  currentDay: number
) {
...
function productionMilestonePriority(
  milestone: Doc<"buildMilestones">,
  currentDay: number
) {
  if (
...
function activeBuildCompletionReviewRecord(
  review: unknown
...
function activeBuildCompletionReviewNote(review: unknown) {
  const value = activeBuildCompletionReviewRecord(review).note;
  return typeof value === "string" ? normalizeOptionalString(value) : undefined;
}

function activeBuildPendingCompletionReview(review: unknown) {
...
function activeBuildCompletionReviewWithSiteVisit(
  review: unknown,
...
async function builderContractorRelationshipScope(
  ctx: QueryCtx | MutationCtx,
  input: {
    brokerageId: Id<"brokerages">;
    contractorId: Id<"contractorProfiles">;
...
function builderVisibleContractorProfile(profile: any) {
  return {
    _id: profile._id,
...
function builderContractorLifecycle(input: {
...
async function ensurePendingBuildAssignmentAcknowledgement(
  ctx: MutationCtx,
  input: {
...
async function resolveBuildAssignmentAcknowledgement(
  ctx: MutationCtx,
  assignmentId: Id<"milestoneContractorAssignments">,
  now: number
) {
...
async function upsertContractorAssignmentDelivery(
  ctx: MutationCtx,
  input: {
    assignmentId: Id<"milestoneContractorAssignments">;
    auth: ActiveBuildDeliveryAuth;
...
type CostItemBudgetTreatment = "add" | "logOnly" | "maintain";
...
function normalizeCostItemBudgetTreatment(
  value: CostItemBudgetTreatment | undefined
): CostItemBudgetTreatment {
  return value ?? "add";
}

function normalizeCostItemBudgetSubmilestoneKey(
...
async function validateProposalCostItemBudgetTarget(
  ctx: QueryCtx | MutationCtx,
  milestone: Pick<Doc<"proposalMilestones">, "_id" | "key">,
  ...
  const budgetSubmilestoneKey = normalizeCostItemBudgetSubmilestoneKey(
    requestedKey
  );
  if (!budgetSubmilestoneKey) {
...
async function applyProposalCostItemBudgetChange(
  ctx: MutationCtx,
  auth: {
    proposal: Doc<"buildProposals">;
    subject: string;
  ...
  const addOperation = (
    ...
    if (!item || normalizeCostItemBudgetTreatment(item.budgetTreatment) !== "add") {
      return;
    }
...
async function validateProposalMaintainedCostCoverage(
  ctx: QueryCtx | MutationCtx,
  input: {
    milestoneId: Id<"proposalMilestones">;
    proposalId: Id<"buildProposals">;
...
async function getActiveBuildDrawRequestOrThrow(
  ctx: QueryCtx | MutationCtx,
  buildId: Id<"activeBuilds">,
  requestKey: string
) {
...
async function calculateActiveBuildAvailableNowCents(
  ctx: QueryCtx | MutationCtx,
  buildId: Id<"activeBuilds">
) {
...
async function validateBuildCostItemBudgetTarget(
  ctx: QueryCtx | MutationCtx,
  milestone: Pick<Doc<"buildMilestones">, "_id" | "key">,
  requestedKey: null | string | undefined,
  treatment: CostItemBudgetTreatment
  ...
  if (!budgetSubmilestoneKey) {
    if (treatment === "maintain") {
      throw new Error(
        "Maintain requires one budget sub-milestone target."
      );
    }
    return;
  }
...
async function validateBuildMaintainedCostCoverage(
  ctx: QueryCtx | MutationCtx,
  input: {
    buildId: Id<"activeBuilds">;
  ...
  const [submilestones, costItems] = await Promise.all([
    ctx.db
      .query("buildSubmilestones")
      .withIndex("by_milestone", (q) => q.eq("buildMilestoneId", input.milestoneId))
      .collect(),
    ctx.db
      .query("buildCostItems")
      .withIndex("by_milestone", (q) => q.eq("buildMilestoneId", input.milestoneId))
...
async function visibleBackofficeDashboardProposalRows(
  ctx: QueryCtx,
  auth: { brokerage: Doc<"brokerages">; roles: RoleSlug[]; subject: string },
...
async function ensureSeedBuilderBrokerAssignment(
  ctx: MutationCtx,
  input: {
    assignedBrokerWorkosUserId: string;
    brokerageId: Id<"brokerages">;
...
async function repairLegacyClosedProposalActiveBuildAggregate(
  ctx: MutationCtx,
  input: {
    auth: {
      brokerage: Doc<"brokerages">;
...
function projectAuditStateChanges(priorState?: string, newState?: string) {
