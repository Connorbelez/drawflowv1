...
export const Route = createFileRoute("/contractor/proposals/$proposalId")({
  staticData: {
    breadcrumb: { label: "Proposal", to: "/contractor/proposals" },
  },
  component: ContractorProposalDetail,
});

/**
 * Scope-limited proposal detail (PRD §8.4). Contractors see only their assigned
 * scope + permit docs + builder contact. Budgets, financing, lender notes, and
 * other contractors' scopes never appear (backend-enforced redaction).
 */
function ContractorProposalDetail() {
  const { proposalId } = Route.useParams();
  const detail = useQuery(api.contractorWorkspace.getContractorProposalDetail, {
  ...
  return (
    <main className="min-h-svh bg-muted/30 p-4 sm:p-6">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-5">
        <header>
          <p className="text-muted-foreground text-xs uppercase">Planning</p>
              ...
              <FramePanel className="flex flex-col gap-2 p-4">
                ...
                {detail.builderContact ? (
                  <div className="text-sm">
                    ...
                    {detail.builderContact.legalName ? (
                      <p className="text-muted-foreground text-xs">
                        {detail.builderContact.legalName}
                      </p>
                    ) : null}
