...
export function buildTimelineShareSnapshotV2(
  ...
  const minimumCashReserve = Math.max(
    0,
...
export function applyTimelineShareSnapshotV2(
  ...
  return {
    ...
    minimumCashReserve: Math.max(
      0,
      Math.round(
        normalizeNumber(
          candidate.minimumCashReserve,
          fallbackState.minimumCashReserve
        )
      )
    ),
...
function normalizeTimelineShareState(
  ...
  return {
    ...
    minimumCashReserve: Math.max(
      0,
      Math.round(normalizeNumber(state.minimumCashReserve, 0))
    ),
    startingCash: Math.max(
      0,
