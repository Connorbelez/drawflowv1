...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Expanded Sub-milestone ops intentionally keep start, completion, evidence upload, assignment empty states, and site-visit ordering on one governed surface.
function SubMilestoneExpandedDetails({
  ...
  const assigneeRequired =
    presentation?.executionOwnership?.state === "assignment_required";
  const assigneeName =
    presentation?.executionOwnership?.assigneeDisplayName ?? null;
  const siteVisit = presentation?.siteVisitRequirement;
  const evidenceCount = presentation?.evidenceCount ?? 0;
  const openRecord = () => onOpenActionItem(item._id);
  const readOnly = !mutationsAllowed;
  ...
  const commandKey = (scope: string) => {
    const key = `${item._id}:${scope}`;
    const existing = commandKeys.current.get(key);
    if (existing) {
      return existing;
    }
    const generated = `${scope}:${globalThis.crypto?.randomUUID?.() ?? Date.now()}`;
    commandKeys.current.set(key, generated);
    return generated;
  };
  const clearCommandKey = (scope: string) => {
    commandKeys.current.delete(`${item._id}:${scope}`);
  };
  ...
  const openStart = () => {
    ...
    setStartRequest({
      ...
      scope: startCommand.scope,
      source: startCommand.source,
      startParent: false,
      submilestoneKey: startCommand.submilestoneKey,
      submilestoneName: startCommand.submilestoneName,
    });
  };
  ...
  const approveChild = async () => {
    ...
    try {
      ...
      clearCommandKey("child-approval");
      toast.success("Sub-milestone approved.");
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
  ...
  const uploadEvidence = async ({ file }: EvidenceUploaderUploadInput) => {
    ...
    <CardPanel className="space-y-0 border-t px-3 py-3 sm:px-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <DetailBlock
          label="Description"
          value={
            startCommand?.submilestoneName
              ? `${startCommand.submilestoneName} on ${startCommand.milestoneName}. Canonical Sub-milestone facts and notes stay in the execution record.`
              : "Canonical description and coordination notes stay in the execution record."
          }
        />
        <DetailBlock
          label="Field notes"
          value={
            presentation?.readyExceptFor?.length
              ? `Ready except for: ${presentation.readyExceptFor.join(", ")}`
              : assigneeRequired
                ? "A responsible operator still needs to accept the Work Allocation."
                : "No outstanding readiness exceptions are projected on this card."
          }
        />
        <DetailBlock
          label="Scope"
          value={
            startCommand
              ? `${startCommand.milestoneName} · ${startCommand.submilestoneName}`
              : item.title
          }
        />
        <div className="grid grid-cols-2 gap-2">
          <CompactFact label="Actual start" value="—" />
          <CompactFact label="Actual end" value="—" />
          <CompactFact
            label="Planned start"
            value={
              presentation?.plannedStartDate
          ...
          <CompactFact
            ...
            value={
              presentation?.plannedCompletionDate
                ? formatPlanDateShort(presentation.plannedCompletionDate)
                : "—"
            }
          />
        </div>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-3">
        ...
        <CompactFact
          label="Forecast"
          value={
            presentation?.completionForecastDate
              ? formatPlanDateShort(presentation.completionForecastDate)
              : "—"
          }
        />
        <CompactFact
          ...
          value={
            presentation?.evidenceReviewState
              ? humanizeEnumLabel(presentation.evidenceReviewState)
              : "—"
          }
        />
      </div>
      ...
      <section className="space-y-2">
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          <AllocationEmptyState
            ctaLabel={
              assigneeName ? "Change assignment" : "Assign tradesperson"
            }
            description={
              assigneeName
                ? "Work Allocation is bound to this Sub-milestone."
                : "No tradesperson is assigned yet. Assignment grants Contractor operate rights; Builders and Admins can still start with permission."
            }
            disabled={readOnly}
            filledLabel={assigneeName}
            highlight={!assigneeName && assigneeRequired}
          ...
          <AllocationEmptyState
            ...
            description="Suppliers are not projected on this card yet. Open the execution record to attach them."
            disabled={readOnly}
          ...
          <AllocationEmptyState
            ...
            description="Materials are not projected on this card yet. Open the execution record to attach them."
            disabled={readOnly}
      ...
      <Separator className="my-3" />
      <SubMilestoneBuilderEvidenceSection
      ...
      <SubMilestoneSiteVisitsSection
        ...
        siteVisit={siteVisit}
      />
      {assigneeRequired || presentation?.attention === "overdue_completion" ? (
      ...
      {startCommand ? (
        <SiteVisitOrderDialog
          build={{ name: startCommand.buildName }}
          milestone={{
            key: startCommand.milestoneKey,
            name: startCommand.milestoneName,
          }}
          onConfirm={confirmSiteVisit}
          onOpenChange={setSiteVisitOpen}
          open={siteVisitOpen}
          request={
            siteVisitOpen ? { milestoneKey: startCommand.milestoneKey } : null
          }
          submilestones={[
            {
