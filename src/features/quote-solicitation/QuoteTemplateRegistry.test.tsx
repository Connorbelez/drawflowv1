...
describe("QuoteTemplateRegistry", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mutationByRef.clear();
    useQuery.mockReturnValue(registry);
  ...
  test("creates the next version on the same template and exposes validation/tax controls", async () => {
    ...
    await waitFor(() => expect(mutationByRef.get(getFunctionName(api.quote_response_templates.updateQuoteResponseTemplateDraft))).toHaveBeenCalled());
  });
});
