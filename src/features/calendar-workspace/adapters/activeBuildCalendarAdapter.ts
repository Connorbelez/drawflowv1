...
export function buildActiveBuildCalendarWorkspaceFromDetail(
  ...
  const organizationId =
    options.organizationId ?? detail.build?.organizationId ?? "visual-fixture";
  ...
  for (const draw of detail.draws ?? []) {
    const startsAt =
      ...
      addDaysIso(baseDate, draw.timingDay ?? 0);
    events.push(
      normalizeCalendarEvent({
        allDay: false,
        ...
        editable: {
          ...
          immutableReason:
            draw.status === "released"
              ...
              : undefined,
          requiredReason:
            draw.status === "released" ? "none" : "materialDecision",
        },
        entity: {
  ...
  if (detail.loanFacility?.paybackDate) {
    events.push(
      normalizeCalendarEvent({
        ...
        status:
          detail.loanFacility.status === "closed" ? "completed" : "planned",
...
export function buildActiveBuildCalendarActions(
  ...
  return [
    ...
    eventAction(
      ...
      "Start milestone work",
      "Record the milestone start.",
      async (context) => {
        if (context.event?.milestoneKey) {
          await actions.startMilestoneWork?.(context.event.milestoneKey);
        }
      },
      Boolean(actions.startMilestoneWork)
    ),
    eventAction(
      "request-milestone-info",
      "Request missing information",
      "Send a milestone information request.",
      async (context) => {
        if (context.event?.milestoneKey) {
    ...
    eventAction(
      "schedule-site-visit",
      "Schedule site visit",
      "Create or assign a site visit window.",
      async (context) => {
        const milestoneKey =
          context.event?.milestoneKey ?? firstMilestoneOnDate(context);
    ...
    eventAction(
      ...
      "Reschedule site visit",
      "Move this site visit to the selected date.",
      async (context) => {
    ...
    eventAction(
      "cancel-site-visit",
      "Cancel site visit",
      "Cancel the site visit with audit reason.",
      async (context) => {
    ...
    eventAction(
      "request-draw",
      "Request draw",
      "Request reimbursement draw.",
      async (context) => {
        if (context.event?.drawGroupKey) {
          await actions.requestDraw?.(context.event.drawGroupKey);
        }
      },
    ...
    eventAction(
      "approve-draw",
      "Approve draw",
      "Backoffice draw approval.",
      async (context) => {
        if (context.event?.drawGroupKey) {
          await actions.approveDraw?.(context.event.drawGroupKey);
        }
      },
    ...
    eventAction(
      ...
      async (context) => {
        if (context.event?.drawGroupKey) {
          await actions.releaseDraw?.(context.event.drawGroupKey);
        }
      },
      Boolean(actions.releaseDraw),
      "Requires Principal Broker/Admin draw-release authority."
    ),
    dateAction(
      "add-evidence-due-date",
      "Add evidence due date",
      "Create an explicit evidence target.",
      async (context) => {
        if (context.date) {
          await actions.setEvidenceDueDate?.({ targetDate: context.date });
        }
      },
    ...
    dateAction(
      ...
      "Create a staff review target date.",
      async (context) => {
        if (context.date) {
          await actions.setReviewTargetDate?.({ targetDate: context.date });
        }
      },
    ...
    dateAction(
      ...
      "Add admin decision target",
      "Create an admin decision target date.",
      async (context) => {
        if (context.date) {
          await actions.setAdminDecisionTargetDate?.({
    ...
    dateAction(
      ...
      "Schedule site visit",
      "Schedule a site visit for a milestone on this day.",
      async (context) => {
...
function defaultCalendarSavedViews(
...
function dateAction(
  ...
  return {
    ...
    availability: enabled
      ? { state: "enabled" }
      : {
          reason: "Requires calendar scheduling authority.",
          state: "disabled",
        },
    description,
    id,
