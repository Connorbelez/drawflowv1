  ...
  test("gives lenders one review queue for milestone, draw, and release decisions", async () => {
    ...
    expect(
      await screen.findByRole("heading", {
        name: "Reject DR-1042 for Harbour Build?",
      }),
    ).toBeTruthy();
    ...
    fireEvent.change(screen.getByLabelText("Reason for rejection"), {
      target: { value: "Invoice does not match verified work." },
    });
    expect(confirmReject.disabled).toBe(false);
    fireEvent.click(confirmReject);
    await waitFor(() =>
      expect(onRejectDraw).toHaveBeenCalledWith({
        reason: "Invoice does not match verified work.",
