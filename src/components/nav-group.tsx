		...
		<SidebarGroup>
			...
			<SidebarMenu>
				{items.map((item) => {
					...
					const childActive =
						item.subItems?.some((sub) => isNavItemActive(sub, pathname)) ?? false;
					return (
						<Collapsible
						...
						>
							{item.subItems?.length ? (
								...
								<NavLeafLink item={item} active={itemActive} />
							)}
						</Collapsible>
					);
				})}
...
function NavLeafLink({ item, active }: { item: SidebarNavItem; active: boolean }) {
	return (
		<SidebarMenuButton
			isActive={active}
...
function NavSubLink({ item, pathname }: { item: SidebarNavItem; pathname: string }) {
