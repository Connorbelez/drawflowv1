...
const springConfig = { stiffness: 240, damping: 28, bounce: 0 };
...
const getAnimationProps = (
  ...
  return {
    ...
    transition: animateIn?.transition,
  };
};
  ...
  (
    ...
    return (
      ...
      <motion.div
        ...
        style={{
          height: smoothHeight,
          overflow: isExpanded && overflowVisible ? "visible" : "hidden",
        }}
...
const ExpandableCard = React.forwardRef<HTMLDivElement, ExpandableCardProps>(
  (
    ...
    const collapsedWidth = collapsedSize.width;
    const collapsedHeight = collapsedSize.height;
    const expandedWidth = expandedSize.width;
    const expandedHeight = expandedSize.height;

    // Create motion values for width and height
    const animatedWidth = useMotionValue(collapsedWidth || 0);
    const animatedHeight = useMotionValue(collapsedHeight || 0);
    ...
    useEffect(() => {
      if (isExpanded) {
        animatedWidth.set(expandedWidth || width);
        animatedHeight.set(expandedHeight || height);
      } else {
        animatedWidth.set(collapsedWidth || width);
        animatedHeight.set(collapsedHeight || height);
      }
    }, [
      isExpanded,
      collapsedWidth,
    ...
    return (
      <motion.div
        className={cn("z-50 cursor-pointer overflow-visible", className)}
        onHoverEnd={handleHoverEnd}
        onHoverStart={handleHover}
        ref={ref}
        style={{
          // Set width and height based on expansion direction
          width: expandDirection === "vertical" ? collapsedWidth : smoothWidth,
          height:
            expandDirection === "horizontal" ? collapsedHeight : smoothHeight,
        }}
            ...
            <div className="grid grid-cols-1 rounded-lg p-1 shadow-md sm:rounded-xl sm:p-1.5 md:rounded-[2rem] md:p-2">
              <div className="rounded-md bg-white p-2 shadow-xl ring-1 ring-border/50 sm:rounded-lg sm:p-3 md:rounded-3xl md:p-4 dark:bg-muted">
                <div className="h-full w-full overflow-hidden">
                  ...
                  <div
                    className={cn("flex h-full flex-col", contentClassName)}
                    ref={measureRef}
                  >
                    {children}
                  </div>
