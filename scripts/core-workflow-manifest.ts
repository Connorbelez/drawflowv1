...
const workflows: ParentWorkflow[] = [
  ...
  },
  {
    ...
    steps: ["Calculate unlocked, reserved, and available value separately from planned forecast rows.", "Builder enters amount, reviews, and submits with idempotent operation ID.", "Operations reviews/prepares recommendation; builder may withdraw while requested.", "Lender Admin approves/rejects requested record.", "Only approved record is released/executed or recorded.", "Record release date/amount/fee treatment and start interest at funds_released.", "Notify builder and create receipt confirmation.", "Builder confirms, reports non-receipt, or reports discrepancy; operations resolves exceptions."],
    inputs: ["Approved milestone drawAvailability, facility principal/balance, reserved requests.", "Requested amount/note/client operation ID.", "Evidence/milestone context, fees, interest implications, release result."],
    outputs: ["Independent activeBuildDrawRequest and immutable request key/history.", "Approval/rejection/withdrawal/release capital event.", "Receipt confirmation/exception task.", "Webhooks/audit/notifications."],
    states: ["request: requested → approved → released; requested → rejected/withdrawn", "draw PRD projection: not_eligible/partially_eligible/pending_review/pending_visit/ready_for_admin/approved_for_release/released/rejected/replanned", "forecast plannedDrawScheduleRows remain planning-only"],
    gates: ["Amount ≤ available now and uses whole cents.", "Operation ID is idempotent and cannot be reused for a different amount.", "Only requested can withdraw/approve/reject; only approved can release.", "Lender Admin final release; interest starts only when funds released."],
    ...
    events: ["draw_request.submitted/approved/rejected/withdrawn", "draw approved_for_release/released", "capital event", "fee/interest start", "receipt confirmed/exception", "audit/outbox/webhooks/notifications"],
    ...
    sources: [`${src.drawRequest} §Domain separation–§Request lifecycle`, `${src.core} §8.1–8.3, §18.3.9–18.3.12`, `${src.notifications} §8`], status: "NEEDS_VALIDATION",
    validationNote: "The newer production request ledger allows builder-selected partial requests against pooled approved-milestone value; the original PRD models Draw Group readiness creating a Draw Release Work Order. Product policy must confirm whether requests consume pooled value, a specific Draw Group, or both, and when operations preparation is mandatory.",
    participants: ["BLDR", "LOPS", "LADM", "SYS"],
    handoffDefinitions: [
      { id: "WF-DRW-001.HO-01", from: "BLDR", to: "LOPS", trigger: "Idempotent draw request is submitted.", artifacts: "Request ID/key, amount, note, available/reserved/unlocked reconciliation, eligible milestone context.", acknowledgement: "Operations claims/reviews and prepares recommendation or requests correction." },
      { id: "WF-DRW-001.HO-02", from: "LOPS", to: "LADM", trigger: "Request/release package is ready for final decision.", artifacts: "Request, approved milestones/evidence summary, amount, fee treatment, facility availability, interest implications, recommendation/warnings.", acknowledgement: "Lender Admin approves, rejects, or leaves blocked with reason." },
    ...
    segments: [
      segment("WF-DRW-001.BLDR.01", "Request reimbursement and confirm receipt", "BLDR", "Builder draws", common({
        ...
        inputs: ["Amount/note/idempotency ID and receipt result."], outputs: ["Request/withdrawal/receipt response."], states: ["requested → withdrawn/approved/rejected → released → receipt confirmed/exception"], gates: ["Draw permission, whole cents, within available balance."], exceptions: ["Over-limit, retry failure preserves input/ID, blocked user receives reason, non-receipt/discrepancy."], permissions: ["Builder Lead/authorized Builder Staff on own Build."],
        upstream: ["WF-MIL-001.BLDR.01"], downstream: ["WF-DRW-001.LOPS.01"], handoffs: ["WF-DRW-001.HO-01", "WF-DRW-001.HO-04", "WF-DRW-001.HO-05"], events: ["request/withdraw/receipt events and notifications"], success: ["Receipt confirmed."], failure: ["Closed rejection/withdrawal or open settlement exception."], sources: [`${src.drawRequest}`], status: "NEEDS_VALIDATION", validationNote: "Draw Group attribution remains unresolved across source documents.",
      })),
      segment("WF-DRW-001.LOPS.01", "Review request and resolve settlement exceptions", "LOPS", "Draw operations", common({
        ...
        upstream: ["WF-DRW-001.BLDR.01"], downstream: ["WF-DRW-001.LADM.01"], handoffs: ["WF-DRW-001.HO-01", "WF-DRW-001.HO-02", "WF-DRW-001.HO-05"], events: ["work claimed/recommended/exception resolved"], success: ["Authority package complete or exception resolved."], failure: ["Blocked/escalated item remains visible."], sources: [`${src.production} §8.9`, `${src.core} §18.3.9–18.3.12`], status: "NEEDS_VALIDATION", validationNote: "Exact relationship between builder request review and legacy Draw Release Work Order is not fully specified.",
      })),
      segment("WF-DRW-001.LADM.01", "Approve, reject, and release draw", "LADM", "Draw authority", common({
        ...
        inputs: ["Release package and decision/release instruction."], outputs: ["Approval/rejection/release authorization."], states: ["requested → approved/rejected; approved → released"], gates: ["Lender Admin only; facility/eligibility; only legal state transitions."], exceptions: ["Availability block, policy concern, execution failure."], permissions: ["Principal Broker/admin final authority."],
      ...
      segment("WF-DRW-001.SYS.01", "Calculate availability and record release", "SYS", "Draw ledger orchestration", common({
        purpose: "Maintain separate forecast/request ledgers, enforce idempotency/availability, record capital once, and drive receipt/webhook events.", trigger: "Availability query, request/decision/release/receipt command.", steps: ["Calculate approved/unlocked/reserved/available.", "Validate/idempotently create request.", "Apply legal decision transition.", "Execute/record release once.", "Record fee/date/interest start.", "Create receipt task and emit events."],
        inputs: ["Milestone/facility/request ledgers and authorized commands."], outputs: ["Request/capital/receipt records and events."], states: ["requested → approved/rejected/withdrawn; approved → released"], gates: ["Forecast never mutates into request; released remains reserved; no duplicate capital event."], exceptions: ["ID mismatch, over-limit, concurrent reservation, execution failure."], permissions: ["System under scoped authorized actor."],
        upstream: ["WF-MIL-001.SYS.01"], downstream: ["WF-COM-001", "WF-INT-001"], handoffs: ["WF-DRW-001.HO-03", "WF-DRW-001.HO-04"], events: ["draw lifecycle/capital/audit/outbox/webhook/notification"], success: ["Ledger reconciles exactly and receipt task exists."], failure: ["No duplicate/partial money-out; actionable failure recorded."], sources: [`${src.drawRequest} §Domain separation–§Request lifecycle`, `${src.core} §18.3.10–11`], status: "NEEDS_VALIDATION", validationNote: "Draw Group attribution/release-work-order policy requires confirmation.",
      })),
    ],
  },
  {
...
function validate(): string[] {
  ...
  const referencedIds = JSON.stringify({ workflows, validationGaps, sourceCoverage }).match(/WF-[A-Z]{3}-\d{3}(?:\.(?:[A-Z]{3,4}\.\d{2}|HO-\d{2})(?:\.STEP-\d{2})?)?/g) ?? [];
