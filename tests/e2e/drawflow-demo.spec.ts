...
async function resetDemo(page: Page) {
  ...
  if (page.url().includes("/proposal")) {
    ...
    await expect(page.getByTestId("workspace-validation-panel")).toContainText(/must finish|exceeds/i);
    await page.keyboard.press("Escape");
  } else {
    await expect(page.getByTestId("milestone-rail-row-foundation")).toContainText("In progress");
  }
}

async function selectRole(page: Page, role: "builderLead" | "lenderAdmin" | "siteVisitor") {
...
async function submitBuilderClaim(page: Page, milestoneId: string, requestedAmount: string) {
  ...
  await expect(page.getByTestId("milestone-detail-sheet")).toContainText("Draft");
  ...
  await expect(page.getByTestId(`milestone-rail-row-${milestoneId}`)).toContainText(/Under review|Evidence submitted/i);
}

async function approveClaim(page: Page, milestoneId: string, needsSiteVisit: boolean) {
  await selectRole(page, "lenderAdmin");
  await openDetail(page, milestoneId);
  await page.getByTestId("accept-evidence").click();
  await expect(page.getByTestId("milestone-detail-sheet")).toContainText("EvidenceApproved");
  if (needsSiteVisit) {
    await page.getByTestId("request-site-visit").click();
    await expect(page.getByTestId("milestone-detail-sheet")).toContainText("SiteVisitRequested");
    await selectRole(page, "siteVisitor");
    await page.getByTestId("claim-site-visit").click();
    await expect(page.getByTestId("milestone-detail-sheet")).toContainText("SiteVisitClaimed");
    await page.getByTestId("submit-site-visit-report").click();
    await expect(page.getByTestId("milestone-detail-sheet")).toContainText("SiteVisitReportSubmitted");
    await selectRole(page, "lenderAdmin");
  }
  await page.getByTestId("approve-milestone").click();
  await expect(page.getByTestId(`milestone-rail-row-${milestoneId}`)).toContainText("Approved");
}
...
async function dragTimelineMilestone(page: Page, milestoneId: string, deltaX: number) {
...
async function dragMarqueeAcrossMilestones(
  ...
  const layerBox = await page.getByTestId("gantt-selection-layer").boundingBox();
  if (!firstBox || !secondBox || !layerBox) {
    throw new Error("Cannot locate visible Gantt milestones and selection layer.");
  }
  ...
  const start = await page.evaluate(
    ({ layer, targetLeft, targetTop, targetBottom }) => {
      ...
      const layerBottom = Math.min(window.innerHeight - 1, layer.y + layer.height);
  ...
  if (!start) {
    throw new Error("Cannot find an empty Gantt background point for marquee drag.");
  }
...
test("IC-ACT-DRAW-GROUP-AUTO-RELEASE release-approves Draw 2 and unblocks Draw 3 capital after final milestone approval", async ({ page }) => {
  ...
  await expect(page.getByTestId("milestone-rail-row-underground_plumbing")).not.toContainText("Blocked");
  ...
  await expect(page.getByTestId("milestone-rail-row-framing")).not.toContainText("Blocked");
  await openDrawer(page, "outbox");
  await expect(page.getByText("demo.drawGroup.releaseApproved")).toBeVisible();
});

test("IC-PROP-GANTT-MULTI-SELECT-BATCH-DRAG shifts selected unlocked proposal milestones", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);
  await expect(page.getByTestId("timeline-milestone-foundation")).toHaveAttribute("data-start-date", "2026-02-04");
  ...
  await page.getByTestId("timeline-milestone-foundation").click({ modifiers: ["Shift"] });
  await page.getByTestId("timeline-milestone-underground_plumbing").click({ modifiers: ["Shift"] });
  await expect(page.getByTestId("gantt-selection-count")).toContainText("2 selected");
  ...
  await expect(page.getByTestId("timeline-milestone-underground_plumbing")).toHaveAttribute("data-selected", "false");
  await page.getByTestId("timeline-milestone-underground_plumbing").click({
    modifiers: [toggleModifier],
  });
  await expect(page.getByTestId("gantt-selection-count")).toContainText("2 selected");
  ...
  await expect(page.getByTestId("timeline-milestone-foundation")).not.toHaveAttribute("data-start-date", foundationBefore.start ?? "");
  await expect(page.getByTestId("timeline-milestone-underground_plumbing")).not.toHaveAttribute("data-start-date", plumbingBefore.start ?? "");
...
test("IC-PROP-GANTT-DRAW-ELIGIBLE-HOVER shows eligible marker details", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);
  ...
  await expect(page.getByTestId("draw-eligible-d7-detail")).toContainText(/Draw value|Total exposure/i);
  const after = await marker.boundingBox();
  expect((after?.height ?? 0) >= (before?.height ?? 0)).toBe(true);
});

test("IC-PROP-GANTT-MARQUEE-SELECT selects intersecting milestones", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);

  await dragMarqueeAcrossMilestones(page, ["foundation", "underground_plumbing"]);

  await expect(page.getByTestId("gantt-selection-marquee")).toHaveCount(0);
  await expect(page.getByTestId("timeline-milestone-foundation")).toHaveAttribute("data-selected", "true");
  await expect(page.getByTestId("timeline-milestone-underground_plumbing")).toHaveAttribute("data-selected", "true");
  await expect(page.getByTestId("gantt-selection-count")).toContainText(/selected/);
  const selectedCount = Number(
    (await page.getByTestId("gantt-selection-count").textContent())?.match(/\d+/)?.[0] ?? 0
  );
  ...
  await expect(page.getByTestId("timeline-milestone-foundation")).toHaveAttribute("data-selected", "false");
});

test("IC-PROP-GANTT-MARQUEE-BATCH-DRAG shifts selected milestones", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);
...
test("IC-PROP-GANTT-MARQUEE does not start from marker labels or milestone bars", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);
  ...
  await page.mouse.down();
  await page.mouse.move(markerBox.x + markerBox.width / 2 + 80, markerBox.y + markerBox.height / 2 + 40, {
    steps: 6,
  });
  await expect(page.getByTestId("gantt-selection-marquee")).toHaveCount(0);
  await page.mouse.up();
...
test("IC-PROP-GANTT-LOCK prevents individual and batch milestone drag", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);
  await expect(page.getByTestId("timeline-milestone-foundation")).toHaveAttribute("data-start-date", "2026-02-04");
  ...
  await expect(page.getByTestId("gantt-sidebar-lock-foundation")).toHaveAttribute("aria-pressed", "true");

  await dragTimelineMilestone(page, "underground_plumbing", 140);
  await expect(page.getByTestId("timeline-milestone-foundation")).toHaveAttribute("data-start-date", foundationBefore.start ?? "");

  const plumbingBefore = await milestoneDates(page, "underground_plumbing");
  await page.getByTestId("timeline-milestone-foundation").click({ modifiers: ["Shift"], force: true });
  await page.getByTestId("timeline-milestone-underground_plumbing").click({ modifiers: ["Shift"] });
  await dragTimelineMilestone(page, "underground_plumbing", 140);
...
test("IC-PROP-DRAW-GROUP-DRAG shifts unlocked milestones and skips locked", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);
  await expect(page.getByTestId("timeline-milestone-underground_plumbing")).toHaveAttribute("data-start-date", "2026-02-19");
  ...
  const eligibleBefore = await page.getByTestId("draw-eligible-d2").boundingBox();

  await dragDrawGroup(page, "d2", 220);

  await expect(page.getByTestId("timeline-milestone-underground_plumbing")).toHaveAttribute("data-start-date", plumbingBefore.start ?? "");
  await expect(page.getByTestId("timeline-milestone-framing")).not.toHaveAttribute("data-start-date", framingBefore.start ?? "");
  await expect.poll(async () => {
    const eligibleAfter = await page.getByTestId("draw-eligible-d2").boundingBox();
    return Math.round((eligibleAfter?.x ?? 0) - (eligibleBefore?.x ?? 0));
  }).not.toBe(0);
});

test("IC-ACT-GANTT-BATCH-FORECAST-SHIFT requires one reason and audits", async ({ page }) => {
  await page.goto("/demo/drawflow/active");
  await waitForWorkspace(page);
  await resetDemo(page);
  await expect(page.getByTestId("timeline-milestone-foundation")).toHaveAttribute("data-start-date", "2026-03-09");
  ...
  await openDrawer(page, "audit");
  await expect(page.getByText("demo_batchMoveMilestoneDates")).toBeVisible();
  await expect(page.getByText("Coordinated weather recovery shift.")).toBeVisible();
});

test("IC-ACT-GANTT-FORECAST-RESIZE-END persists forecast shift and exposes audit entry", async ({ page }) => {
  await page.goto("/demo/drawflow/active");
  await waitForWorkspace(page);
  await resetDemo(page);
  await selectMilestone(page, "foundation");

  await expect(page.getByTestId("timeline-foundation-right-resize-handle")).toBeVisible();
  await openDetail(page, "foundation");
  await page.getByTestId("audit-reason-input").fill("Weather delay moved the pour window.");
  await page.getByTestId("milestone-start-date-input").fill("2026-03-16");
  await page.getByTestId("save-milestone").click();

  await openDrawer(page, "audit");
  await expect(page.getByText("demo_updateForecastDatesWithReason")).toBeVisible();
});

test("IC-ACT-EVIDENCE-UPLOAD-METADATA supports upload metadata and completion outbox flow", async ({ page }) => {
  await page.goto("/demo/drawflow/active");
  await waitForWorkspace(page);
  await resetDemo(page);
  ...
  await page.getByTestId("submit-completion-report").click();
  await expect(page.getByTestId("milestone-rail-row-water_sewer")).toContainText(/Under review|Evidence submitted/i);
  await openDrawer(page, "outbox");
  await expect(page.getByTestId("workspace-outbox-drawer")).toContainText("demo.milestone.completionClaimSubmitted");
});

test("IC-PROP-APPLY-RECOMMENDED-PLAN clears hard errors and IC-PROP-SUBMIT-SUCCESS freezes proposal", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);

  await openValidation(page);
  await expect(page.getByTestId("workspace-validation-panel")).toContainText(/exceeds|must finish/i);
  await page.keyboard.press("Escape");
  await page.getByTestId("proposal-apply-plan").click();
  await openValidation(page);
  await expect(page.getByTestId("workspace-validation-panel")).toContainText("No blocking errors");
  await page.keyboard.press("Escape");
  await expect(page.getByTestId("proposal-submit")).toBeEnabled();
  await page.getByTestId("proposal-submit").click();

  await expect(page.getByText("Proposal submitted for review. Approval workflow is out of scope for this demo.")).toBeVisible();
...
test("IC-PROP-EDIT-VALUE updates validation and dependency controls enforce system rules", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);
  ...
  await page.getByTestId("save-milestone").click();
  await expect(page.getByTestId("milestone-rail-row-foundation")).toContainText("$900K");
  await openValidation(page);
  await expect(page.getByTestId("workspace-validation-panel")).toContainText(/exceeds/i);
  await page.keyboard.press("Escape");
  ...
  await expect(page.getByTestId("dependency-error")).toContainText("System hard dependencies cannot be removed.");
...
test("IC-PROP-ADD-MILESTONE creates editable custom milestone and supports rail reorder", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);
...
test("workspace issue chips open popouts, quick fixes audit, and dismissals are condition-aware", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);

  await page.getByTestId("milestone-issue-chip-foundation").click();
  await expect(page.getByTestId("milestone-issue-chip-foundation-popover")).toContainText(/Why it matters|Recommended fix/i);
  await page.locator('[data-testid^="issue-focus-"]').first().click();
  await expect(page.getByTestId("milestone-detail-sheet")).toBeVisible();

  await openValidation(page);
  await page.getByTestId("validation-issue-draw-overlap-d1-d2").click();
  await expect(page.getByTestId("validation-issue-draw-overlap-d1-d2-popover")).toContainText("Draw groups overlap");
  await page.getByTestId("issue-dismiss-draw-overlap-d1-d2").click();
  await expect(page.getByTestId("validation-issue-draw-overlap-d1-d2")).toHaveCount(0);
  await page
  ...
  await page.getByTestId("save-milestone").click();
  await openValidation(page);
  await expect(page.getByTestId("validation-issue-draw-overlap-d1-d2")).toBeVisible();

  await page.locator('[data-testid^="validation-issue-dependency-order-"]').first().click();
  await page.locator('[data-testid^="issue-quickfix-dependency-order-"]').first().click({ force: true });
  await page.keyboard.press("Escape");
  await openDrawer(page, "audit");
  await expect(page.getByText("WorkspaceIssueQuickFixApplied")).toBeVisible();
});

test("proposal rail collapses and drag handle reorders milestones persistently", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);
  ...
  const beforeIds = await page.locator('[data-testid^="milestone-rail-row-"]').evaluateAll((rows) =>
    rows.map((row) => row.getAttribute("data-testid"))
  );
  const beforeIndex = beforeIds.indexOf("milestone-rail-row-foundation");
  await expect(page.getByTestId("milestone-drag-handle-foundation")).toBeEnabled();
  ...
  await page.reload();
  await waitForWorkspace(page);
  const afterIds = await page.locator('[data-testid^="milestone-rail-row-"]').evaluateAll((rows) =>
    rows.map((row) => row.getAttribute("data-testid"))
  );
  expect(afterIds.indexOf("milestone-rail-row-foundation")).not.toBe(beforeIndex);
});

test("thin Gantt bars expose preview, detail sheet, and anchored issue popouts", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);

  await expect(page.getByTestId("timeline-milestone-foundation")).toContainText(/M-050|Foundation/);
  await expect(page.getByTestId("timeline-milestone-foundation")).not.toContainText("$");
  ...
  await expect(page.getByTestId("gantt-issue-chip-foundation-popover")).toContainText(/Recommended fix/i);
  await page.getByTestId("timeline-milestone-foundation").click();
  await expect(page.getByTestId("milestone-detail-sheet")).toBeVisible();
});

test("JIT compilation updates validation and draw eligibility without Analyze", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);
  ...
  const initialEligibleText = await page.getByTestId("draw-eligible-d1").textContent();
  await openDetail(page, "foundation");
  await page.getByTestId("milestone-duration-input").fill("80");
  await page.getByTestId("milestone-duration-input").blur();
  await page.getByTestId("save-milestone").click();
  await expect(page.getByTestId("milestone-rail-row-foundation")).toContainText("80d");
  await openValidation(page);
  await expect(page.getByTestId("workspace-validation-panel")).toContainText(/overlap|must finish|exceeds/i);
  await page.keyboard.press("Escape");
  const shiftedEligibleText = await page.getByTestId("draw-eligible-d1").textContent();
  expect(shiftedEligibleText).not.toBe(initialEligibleText);
});

test("IC-PROP-DRAW-GROUP-SPLIT-MERGE recomputes draw overlays and fee summary", async ({ page }) => {
  await page.goto("/demo/drawflow/proposal");
  await waitForWorkspace(page);
  await resetDemo(page);

  await openDetail(page, "temp_fencing");
  const initialOverlayCount = await page.locator('[data-testid^="draw-label-"]').count();
  await page.getByTestId("split-draw").click();
  await expect(page.locator('[data-testid^="draw-label-"]')).toHaveCount(initialOverlayCount + 1);
  await expect(page.getByText(/fees/i).first()).toBeVisible();
  await openDetail(page, "foundation");
  await page.getByTestId("merge-prev-draw").click();
  await expect(page.locator('[data-testid^="draw-label-"]')).toHaveCount(initialOverlayCount);
});

test("IC-SHARED-DRAWER-AUDIT-OUTBOX exposes append-only records", async ({ page }) => {
  await page.goto("/demo/drawflow/active");
  await waitForWorkspace(page);
  await resetDemo(page);
  ...
  await page.getByTestId("submit-completion-report").click();
  await openDrawer(page, "outbox");
  await expect(page.getByText("demo.milestone.completionClaimSubmitted")).toBeVisible();
});

test("DrawFlow workspace screenshot QA across responsive breakpoints", async ({ page }) => {
  for (const [name, viewport] of [
    ...
    await page.goto(name === "mobile" ? "/demo/drawflow/proposal" : "/demo/drawflow/active");
    await waitForWorkspace(page);
    await expect(page.getByTestId("build-workspace-shell")).toBeVisible();
    ...
    expect(Math.abs(viewportFit.ganttBottom - viewportFit.viewportBottom)).toBeLessThanOrEqual(2);
    await page.screenshot({ fullPage: true, path: `test-results/drawflow-${name}.png` });
  }
});
